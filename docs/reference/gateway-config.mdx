...
The gateway emits an OCSF audit event for every state-changing operation — workspace, member, provider, sandbox, credential, policy, and settings mutations — carrying the authenticated principal as the actor. Audit events are on by default and ride the same export pipeline as other gateway events. `[openshell.gateway.audit]` controls the master switch and the record depth; omit the table to keep the defaults.
