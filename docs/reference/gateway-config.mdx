...
# Audit event depth toggles. Omit the table to keep the defaults shown here.
...
The gateway emits an OCSF audit event for every state-changing operation — workspace, member, provider, sandbox, credential, policy, and settings mutations — carrying the authenticated principal as the actor. Audit events are always on and ride the same export pipeline as other gateway events. `[openshell.gateway.audit]` controls their depth, not their existence; omit the table to keep the defaults.
...
`auth_success_events` adds a success event for every authenticated request, producing a complete authentication ledger for deployments that accept the volume. Authentication failures are always emitted.
