...
The gateway's own fan-in sustains a few hundred thousand lines per second, and the buffer drains as fast as the collector acknowledges batches, so a `telemetry_gap` usually points at collector capacity or the network rather than the gateway.

Sandboxes push OCSF events with the schema flattened into dotted `ocsf.*` attributes alongside the rendered summary, so a SIEM can match a field instead of pattern-matching the line:
