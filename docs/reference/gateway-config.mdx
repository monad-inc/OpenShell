...
Log export is bounded by the SDK's log record queue, not by the gateway. The queue holds `OTEL_BLRP_MAX_QUEUE_SIZE` records — **2048** by default — and once it fills, the SDK drops records and reports `BatchLogProcessor.LogDroppingStarted` at WARN. A gateway aggregating several busy sandboxes can produce lines faster than a distant collector accepts them, so raise the queue and the batch size before running one in production:

```shell
OTEL_BLRP_MAX_QUEUE_SIZE=65536 OTEL_BLRP_MAX_EXPORT_BATCH_SIZE=2048
```

The gateway's own fan-in sustains a few hundred thousand lines per second, and the queue drains as fast as the collector acknowledges batches. Watch for that WARN: it is the signal that export, rather than the sandbox, is the limiting stage.
...
| `OTEL_BLRP_SCHEDULE_DELAY`, `OTEL_BLRP_MAX_QUEUE_SIZE`, `OTEL_BLRP_MAX_EXPORT_BATCH_SIZE`, `OTEL_BLRP_EXPORT_TIMEOUT` | Batch **log** record processor tuning. Separate from the span knobs above; `OTEL_BSP_*` has no effect on log export. |
