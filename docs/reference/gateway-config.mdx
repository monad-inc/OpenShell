...
The export ceiling depends on the shape OCSF events arrive in, because per-record cost at every stage is dominated by attribute count rather than bytes. As reference points, on commodity dev hardware (`cargo bench --bench log_fanin --bench log_export`), a 46-field production denial costs about 4.5 µs to publish through the fan-in bus and the export worker drains roughly 70 thousand lines per second in the default flattened format; pushed as raw JSON (`OPENSHELL_OCSF_PUSH_FORMAT=raw`, below) the same event publishes in ~0.4 µs and drains at roughly 290 thousand lines per second. Plain log lines without OCSF fields drain at over a million lines per second.
...
**`flat`** (the default) flattens the event schema into dotted `ocsf.*` attributes alongside the rendered summary, so a SIEM can match a field instead of pattern-matching the line:
...
Nested objects join with `.`, arrays of scalars join with `,` on one key, arrays of objects are indexed (`ocsf.affected.0.name`), absent fields are omitted rather than exported empty, and a single value is truncated at 256 characters.

**`raw`** pushes the complete OCSF document as a single `ocsf.raw` attribute — the same JSON the OCSF JSONL file carries, with nothing flattened, joined, or truncated — plus `ocsf.severity_id` beside it. This is the high-throughput shape: per-record cost across the sandbox, the gateway, and the exporter is dominated by attribute count, not bytes, and one attribute instead of ~46 raises the gateway's export ceiling several-fold (see Export capacity). The trade is that field-level structure is recovered downstream instead of being present on the record; an OpenTelemetry Collector restores it with a transform such as:
...
Prefer `raw` when the collector or SIEM can parse JSON and the gateway aggregates many busy sandboxes; prefer `flat` when records must arrive query-ready.

In both shapes `ocsf.severity_id` sets the record's OTLP severity, so denials and findings outrank routine events regardless of `ocsf_full_payload`, and unrecognized values of `OPENSHELL_OCSF_PUSH_FORMAT` fall back to `flat`.
