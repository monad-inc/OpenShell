...
- **A new state-changing gateway RPC must emit an audit event** (`EntityManagementBuilder` or `ConfigStateChangeBuilder`) carrying the authenticated principal as the OCSF actor. The audit event taxonomy in the logging architecture doc maps existing handlers. Route the emission through the `audit` module helpers (`emit_entity_outcome` / the settings emitter), never raw `ocsf_emit!` — the helpers apply the `[openshell.gateway.audit] enabled` master toggle (`OPENSHELL_AUDIT_EVENTS`), which must govern every audit emission.
