...
- **A new state-changing gateway RPC must emit an audit event** (`EntityManagementBuilder` or `ConfigStateChangeBuilder`) carrying the authenticated principal as the OCSF actor. The audit event taxonomy in the logging architecture doc maps existing handlers. Route the emission through the `audit` module helpers (`emit_entity_outcome`, `emit_entity`, `emit_config_outcome`, the settings emitter; `emit_system_entity` with a `system:<component>` actor for background mutators with no request principal), never raw `ocsf_emit!` — the helpers apply the `[openshell.gateway.audit] enabled` master toggle (`OPENSHELL_AUDIT_EVENTS`), which must govern every audit emission. Failed mutations emit at `Low` severity, successes at `Informational`.
